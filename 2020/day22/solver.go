...
func Solve() (result int) {
	p1 := player{utility.IntArr(strings.Split(utility.Input2020Day22(), "\n\n")[0], "\n")}
	p2 := player{utility.IntArr(strings.Split(utility.Input2020Day22(), "\n\n")[1], "\n")}
...
func getResult(winner player) (result int) {
	for i := len(winner.handCards); i > 0; i-- {
		result += i * winner.handCards[len(winner.handCards)-i]
	}
	return
}

func play(p1 *player, p2 *player) (winner player) {
	for {
		if len((*p2).handCards) == 0 {
			return *p1
		}
		if len((*p1).handCards) == 0 {
			return *p2
		}
		combat(p1, p2)
	}
